...
type TripLeg struct {
	...
	OriginAddress    string       `json:"origin_address" gorm:"not null" binding:"required"`
	OriginLat        float64      `json:"origin_lat"`
	OriginLng        float64      `json:"origin_lng"`
	DestAddress      string       `json:"dest_address" gorm:"not null" binding:"required"`
	DestLat          float64      `json:"dest_lat"`
	DestLng          float64      `json:"dest_lng"`
	DepartureAt      time.Time    `json:"departure_at" gorm:"index:idx_trip_legs_departure"`
	ArrivalAt        time.Time    `json:"arrival_at"`
	LegType          string       `json:"leg_type" gorm:"type:varchar(20);default:outbound;not null"`
	CargoStatus      string       `json:"cargo_status" gorm:"type:varchar(20);default:loaded;not null;index:idx_trip_legs_visibility_cargo"`
	...
	Visibility       string       `json:"visibility" gorm:"type:varchar(20);default:private;not null;index:idx_trip_legs_visibility_cargo"`
	...
	Status           string       `json:"status" gorm:"type:varchar(20);default:scheduled;not null;index:idx_trip_legs_status"`
